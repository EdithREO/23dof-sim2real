...
void PhysicsThread(mj::Simulate *sim, const char *filename)
{
  ...
  if (filename != nullptr)
  {
    ...
    if (dnew)
    {
      ...
      std::printf("[unitree_mujoco] stand keyframe nkey=%d elastic_band=%d\n",
                  m->nkey, param::config.enable_elastic_band);
      std::printf("[unitree_mujoco] qpos z=%.4f quat=%.4f %.4f %.4f %.4f hinge0=%.4f\n",
                  d->qpos[2], d->qpos[3], d->qpos[4], d->qpos[5], d->qpos[6], d->qpos[7]);
      std::fflush(stdout);
