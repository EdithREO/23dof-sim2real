...
public:
    ...
    virtual void run()
    {
        ...
        // lowcmd
        {
            ...
            const bool no_sdk_cmd = lowcmd->isTimeout();
            // Before SDK publishes LowCmd (or after it times out): hold stand keyframe.
            if(no_sdk_cmd) {
                if(!hold_q_init_) {
                    hold_q_.assign(num_motor_, 0.0);
                    for(int i(0); i<num_motor_; i++) {
                        int jid = mj_model_->actuator_trnid[2 * i];
                        int qadr = mj_model_->jnt_qposadr[jid];
                        hold_q_[i] = mj_data_->qpos[qadr];
                    }
                    hold_q_init_ = true;
                    std::cout << "[bridge] latched stand hold targets (waiting for rt/lowcmd)"
                              << std::endl;
                }
                const double hold_kp = 200.0;
                const double hold_kd = 5.0;
                for(int i(0); i<num_motor_; i++) {
                    int jid = mj_model_->actuator_trnid[2 * i];
                    int qadr = mj_model_->jnt_qposadr[jid];
                    int vadr = mj_model_->jnt_dofadr[jid];
                    mj_data_->ctrl[i] =
                        hold_kp * (hold_q_[i] - mj_data_->qpos[qadr]) -
